#!/bin/bash
# Wrapper script used to call aurweb-notify from git hooks
# when utilizing a Poetry-based virtualenv.
# Based on aurweb-git-auth.sh wrapper pattern

# Debug logging
exec 2>>/tmp/aurweb-notify-debug.log
echo "=== $(date) ===" >&2
echo "Args: $*" >&2
echo "User: $(id)" >&2
echo "HOME: $HOME" >&2
...
if [[ -n "$venv_path" && -f "$venv_path/bin/python" ]]; then
	# Try calling aurweb-notify entry point directly from venv
	echo "Trying to call aurweb-notify entry point from venv" >&2
	if [[ -f "$venv_path/bin/aurweb-notify" ]]; then
		echo "Found entry point at $venv_path/bin/aurweb-notify" >&2
		"$venv_path/bin/aurweb-notify" "$@" 2>&1
		exit $?
	else
		echo "No entry point found, trying Python module..." >&2
		"$venv_path/bin/python" -m aurweb.scripts.notify "$@" 2>&1
		exit $?
	fi
else
	echo "Could not find virtualenv, falling back to poetry run" >&2
	poetry run aurweb-notify "$@" 2>&1
	exit $?
fi
